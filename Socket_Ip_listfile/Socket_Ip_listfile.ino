...
IPAddress IpServer(192, 168, 1, 102);  // Statische Adresse der Server 

// hier wird versuch , ein statische IP Adress für den Arduino festzusetzen 
IPAddress IpArduino(192, 168, 1, 101); // diese Adress muss geandert werden , je nachdem Ihre Router konfiguration
EthernetClient Socket_client;
int Port_Remote_Server = 700; 
...
void loop()
{
 while (Socket_client.connected()) {
    ...
    Socket_client.stop(); // mussen wir  zuert die vorhirige Verbindung schliessen und eine neue bauen
    // delay(1000);
    delay(10000); // jede 10 sekunde 
